...
addNews.forEach((el) => {
  const { author, title, content, urlToImage, readMoreUrl, publishedAt, id } =
    el;
  loadBox(author, title, content, urlToImage, readMoreUrl, publishedAt, id);
});
let count = 0;
...
// -----------------------------------get data api--------------------------------------
async function getData(topicNews='latest') {
  allNews.innerHTML = "";
  fetch(
    `https://newsapi.org/v2/everything?q=${topicNews}&apiKey=e06ede8d77b84d84b7ef43da281086a9`
  )
...
function takeValue(el) {
  const { author, title, content, urlToImage, readMoreUrl, publishedAt, id } =
    el;
  createBox(author, title, content, urlToImage, readMoreUrl, publishedAt, id);
}
